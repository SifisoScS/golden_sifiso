...
This version includes robust form-based authentication for UI integration tests.
"""
...
def extract_csrf_token(html_content):
    ...
    # Look for the CSRF token in a hidden input field
...
def test_user_interface_integration():
    ...
    # Perform a proper login with CSRF token
    print("Performing login with CSRF token...")
    
    # Step 1: Get the login page to extract CSRF token
    login_response = client.get('/login')
    csrf_token = extract_csrf_token(login_response.data.decode('utf-8'))
    ...
    if "dashboard" in response.data.decode('utf-8').lower():
    ...
    if subject:
        ...
        if topic:
            response = client.post("/lessons/generate/lesson", data={
                ...
                "grade": 4,
                "difficulty": "intermediate",
                "csrf_token": csrf_token  # Include CSRF token for POST request
            })
...
def run_all_tests():
    ...
    with open(os.path.join(output_dir, 'lessons_platform_test_results_with_robust_auth.txt'), 'w') as f:
        f.write(f"Lessons Platform Test Results - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        for name, result in results.items():
            f.write(f"{name}: {result}\n")
        f.write(f"\nOverall result: {'PASSED' if all_passed else 'FAILED'}\n")
    
    print(f"Test results saved to '{output_dir}/lessons_platform_test_results_with_robust_auth.txt'")
