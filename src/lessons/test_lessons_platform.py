...
This script tests the end-to-end functionality of the Lessons Platform,
including content generation, storage, retrieval, and UI integration.
"""
...
def run_all_tests():
    ...
    with open(os.path.join(output_dir, 'lessons_platform_test_results.txt'), 'w') as f:
        f.write(f"Lessons Platform Test Results - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
        for name, result in results.items():
            f.write(f"{name}: {result}\n")
        f.write(f"\nOverall result: {'PASSED' if all_passed else 'FAILED'}\n")
    
    print(f"Test results saved to '{output_dir}/lessons_platform_test_results.txt'")
